...
// Graphing loss vs. iterations
var lossGraph = new cnnvis.Graph({"step_horizon": 5000});

lossGraph.id = "lossGraph";
$("#loss-graph").html(lossGraph);
var lossData = []

function graphLoss(loss, iteration, scheduled){
  if (iteration <= 5000) {
    if (scheduled && iteration % 1000 == 0 && iteration != 0) {
      trainer.learning_rate /= 2.0;
      console.log("LEARNING RATE: ", trainer.learning_rate);
    }
    lossGraph.add(iteration, loss);
    lossGraph.drawSelf($("#loss-graph")[0]);
    lossData.push(loss);
  }
}
...
function update(){
  ...
  graphLoss(smooth_loss, counter, true);
}
